...
;						SHIFT15);						, int
;
;******* THESE FIGURES have been improved 16-Oct-18, see below : *****
; resources : prolog 9, loop N*7, epilog 6
; OLD : total cycles 15 + N*7 plus overhead from stack handling for 5 variables
; OLD : example : N=138
; OLD :        via formula   981,
; OLD :        via profiling 1011
;**********************************************************************
;
; prototype:
; ==========
...
;
;**********************************************************************
; 16-Oct-18 : loop: is now completely empty, "[A1] sub a1,1,a1" is now done in "b loop" pipeline
;
;**********************************************************************
; old resources : prolog 9, loop N*7, epilog 6
; new resources : prolog 9, loop N*6, epilog 6
; total cycles 16 + N*6 plus overhead from stack handling for 5 variables
; example : N=138
;        via formula   843,
;        via profiling 855 verified 16-Oct-2018
;**********************************************************************
;
;
...
; remark : a10 and a12 are NOT used any more. problems with cfftr2.asm !!
		.def	FIR_filter_sc
FIR_filter_sc:			    ; ASM function called from C
; prolog (warm up)
...
; pre-load ONLY first delay/coeff (done again at the end of "b loop;" below !!
   ...
   ||	ldh 	*b4--,b2	; B2=h[N-1-i] i=0,1,...,N-1
		...
		sub 	a1,1,a1		; pre-decrement loop count
; loop kernel (cycle)
loop:					    ; start of FIR loop  
  [A1]	b   	loop		; branch to loop if count # 0
 		mpy 	a2,b2,a6	; A6=x[n-(N-1)+i]*h[N-1-i] 			// 1a
 ||		sub 	a1,1,a1		; decrement loop count				// 1b
 		ldh 	*a4++,a2	; A2=x[n-(N-1)+i] i=0,1,...,N-1		// 2a
 ||		ldh 	*b4--,b2	; B2=h[N-1-i] i=0,1,...,N-1			// 2b
		sth 	a7,*-a4[2]	; -->x[(n-(N-1)+i] update sample	// 3
		ldh 	*a4,a7      ; A7=x[(n-(N-1)+i+1]update delays	// 4
 		add 	a6,a5,a5	; accumlate in a5 					// 5
